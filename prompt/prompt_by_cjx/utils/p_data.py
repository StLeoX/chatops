...
def pre_process_data_by_window_item(data, window_size):
    ...
    # 检测峰值
    # 这里我们使用简单的方法：如果一个点比前一个和后一个点都大，我们认为它是一个峰值
    ...
    for drill in file_obj:
        ...
        for M in drill['playPodInfo']:
            ...
            for index in M['playIndexInfo']:
                index['data'] = pre_process_data_by_window_item(index['data'], 15)
    return file_obj
...
def reformObj(data):
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            converted_dict[key] = reformObj(value)  # 递归调用处理嵌套的字典或列表
        return converted_dict
    elif isinstance(data, list):
        converted_list = []
        for item in data:
            converted_list.append(reformObj(item))  # 递归调用处理嵌套的字典或列表
